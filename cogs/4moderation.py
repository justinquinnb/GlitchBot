...
class Moderation(commands.Cog):
  ...
  #async def lockdown(self, ctx, inputCode:str= None):
    ...
    try:
      ...
      if possible:
        ...
        if (approves == disapproves) and (approves > 1 and disapproves > 1):
          ...
          modResultsEmbed = discord.Embed(title="There was a tie!", description=f"{user.display_name} will not be banned.", color=colors["GGpurple"])
          ...
          # Inform the mods of the final decision using an embed
          modResultsEmbed = discord.Embed(title="Nobody responded!", description=f"{user.display_name} will not be banned.", color=colors["GGpurple"])
          modResultsEmbed.set_thumbnail(url="https://cdn.discordapp.com/attachments/796907538570412033/804384663728422952/memberbanautocontroversial.png")
          await modChannel.send(embed=modResultsEmbed)

        elif approves > disapproves:
          # Inform the mods of the final decision using an embed
          modResultsEmbed = discord.Embed(title="Ban approved!", description=f"{user.display_name} will be banned!", color=colors["GGred"])
          ...
          # Inform the mods of the final decision using an embed
          modResultsEmbed = discord.Embed(title="Ban not approved!", description=f"{user.display_name} will not be banned!", color=colors["GGblue"])
  ...
  async def warn(self, ctx, user: discord.Member, reason: str):
    ...
    if ((user.top_role.name != "Mods") and (user.top_role.name != "Admins")) and user.top_role.name != "Owners":
      ...
      if db["Warnings For " + str(user.id)] >= 5:
        await ctx.send(f"**This is {user.display_name}'s {db['Warnings For ' + str(user.id)]} warning!** Though not required, it may be time to consider further action.")
    else:
  ...
  async def report(self, ctx, user: discord.Member, reason: str):
    ...
    if ((user.top_role.name != "Mods") and (user.top_role.name != "Admins")) and user.top_role.name != "Owners":
      ...
      reportEmbed = discord.Embed(title=f"{user.display_name} was reported for:", description=reason, color=colors["GGred"])
